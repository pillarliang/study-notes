...
# Execution 层：执行环境与沙箱（Execution Environment & Sandbox）
...
- **OSWorld**——基于 VM 的环境，同时也被 [[06-V-验证与评估|Verification 层]] 当评估基准用
...
- 这种"双角色"和 §2.2 桌面 computer-use 不同（那个操作的是整个桌面、不是浏览器），并且**直接把 Execution 层和 [[06-V-验证与评估|Verification 层]] 接在一起**。
