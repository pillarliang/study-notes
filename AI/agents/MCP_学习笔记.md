...
上图是**一次性全量发现**——`tools/list` 把 Server 注册的所有工具一次性返回给 LLM。当工具数量从几个增长到几十甚至上百,这种模式会出现上下文挤占和决策精度下降的问题,需要切换到动态加载模式,见第 12 节。
...
其中 `notifications/tools/list_changed` 是动态工具加载模式的核心机制,见第 12 节。
...
4.1 节描述的标准发现流程是**一次性全量返回**——`tools/list` 把 Server 注册的所有工具一次性交给 LLM。当工具数量从几个增长到几十甚至上百,这种模式会出现两个代价:

- **上下文成本**:每个工具的 schema 都占 token,工具多了会挤占大量上下文窗口,留给对话本身的空间被压缩。
- **决策精度**:工具选择本质是 LLM 在做注意力分配,候选项越多,选错的概率越高。

### 12.2 关键概念:工具表与元工具

要理解动态加载,先把两个抽象概念落到实处。

**工具表**:Server 内部维护着一张表,key 是工具名,value 是工具实现。`tools/list` 把全部工具列出来给 Client。所以"**动态注册/注销工具**"没有任何魔法——就是 Server 在运行时往这张表里加(或删)条目而已。Server 处理 `tools/list` 的逻辑是**完全无状态**的——每次都读当前工具表、返回所有 key,不维护计数器、不区分"第几次"。两次返回结果不同,纯粹是因为中间有元工具改了工具表的内容(跟 HTTP `GET /users` 同构:接口不变,数据源变了)。

**元工具(Meta-tool)**:**一个"用来往工具表里加工具"的工具**——本身像普通业务工具一样用 `@mcp.tool()` 注册,只不过调用时执行的是 `mcp.add_tool(...)`,而不是业务逻辑。(下面流程图与 12.4 例子里的"工具加载器"就是它。)元工具的数量没有定数:可以是 1 个统一加载器,也可以按类别拆成多个(数据库加载器、文件加载器、网络加载器各管一摊),甚至跟一部分常用业务工具一起在启动时就暴露——笔记的例子取 1 个,只是最简形态。

**会话隔离**:工具表是**按会话独立**的,不同用户之间互不影响。stdio 模式下每个 Client 启动独立的 Server 子进程,天然进程隔离;Streamable HTTP 模式下 Server 通过 `Mcp-Session-Id`(第 6.2 节)区分会话,每个会话各自维护工具表状态。A 的元工具调用只改 A 的工具表,B 新建会话时拿到的仍是初始状态。

把三者串起来:元工具被调用时,Server 修改**当前会话**的工具表,然后发 `notifications/tools/list_changed`(第 7 节);Client 收到后重新调 `tools/list`,新工具就同步到 LLM 那边了。

### 12.3 完整流程

假设一个 MCP Server 背后管着数据库、文件系统、外部 API 三大块能力,总共 60 多个业务工具。启动时它不暴露任何业务工具,只暴露 3 个元工具——**数据库加载器**、**文件加载器**、**API 加载器**,各负责加载自己领域的业务工具。
...
sequenceDiagram
    ...
    Note over Client,Server: 第一次发现:只暴露 3 个元工具
    Client->>Server: tools/list
    Server-->>Client: [数据库加载器, 文件加载器, API 加载器]

    Client->>LLM: 提供 3 个元工具
    LLM->>LLM: 判断任务需要数据库能力
    LLM->>Client: 请求调用数据库加载器("postgres")
    Client->>Server: tools/call

    Note over Server: 往工具表里加入 postgres 相关业务工具
    Server-->>Client: 加载完成
    Server--)Client: notifications/tools/list_changed

    Note over Client,Server: 第二次发现:拿到扩展后的工具集
    Client->>Server: tools/list
    Server-->>Client: [3 个元工具 + 数据库查询, 表结构查看, ...]

    Client->>LLM: 更新可用工具列表
    LLM->>LLM: 从数据库工具中决策
    LLM->>Client: 请求调用数据库查询
    Client->>Server: tools/call
    Server-->>Client: 查询结果
    Client->>LLM: 回传结果
```
...
- 实线箭头(`->>`)是请求,虚线箭头(`-->>`)是响应——跟 4.1 节约定一致
- **虚线开口箭头(`--)`)是通知**,对应 2.3 节的第三种 JSON-RPC 消息(无 `id`,不需回复)

跟 4.1 节标准流程对比,差异只有两处:

1. **第一次 `tools/list` 返回的只是元工具,而不是全部业务工具**——LLM 看到 3 个而不是 60 个
2. **多了一轮"元工具调用 → 列表变更通知 → 重新拉取"的循环**

文件加载器和 API 加载器始终留在工具表里,但 LLM 不需要调它们——不用的元工具不占什么成本(schema 很小)。后续任务需要读文件时,LLM 再调文件加载器,同样的循环再走一次。

### 12.4 上面流程的逐步拆解

用户向 LLM 提问"帮我看一下 postgres 里 users 表的结构"。下面是 Client 与 Server 之间发生的事:

1. **Client 发 `tools/list`。** Server 返回工具表当前内容——3 个元工具:数据库加载器、文件加载器、API 加载器。
2. **Client 把这 3 个工具交给 LLM。** LLM 看到任务跟数据库相关,选择调用数据库加载器。
3. **Client 发 `tools/call` 调用数据库加载器,参数 `{database: "postgres"}`。** Server 根据参数值决定注册哪批工具——传 `"postgres"` 就塞 postgres 的查询和表结构工具,传 `"mysql"` 就塞 mysql 的。这里塞了两个:**数据库查询** 和 **表结构查看**。工具表从 3 项变成 5 项。
4. **Server 做两件事:**
  - 把加载器的返回值("已加载 postgres 工具")作为响应回给 Client;
  - 发一条 `notifications/tools/list_changed`,告知 Client 工具表变了。
5. **Client 收到通知,再发一次 `tools/list`。** Server 返回 5 个工具(3 个元工具 + 2 个业务工具)。
6. **Client 把更新后的列表交给 LLM。** LLM 调用"表结构查看"完成任务。

**关键点**:LLM 始终只看到 3~5 个工具,而不是 Server 实际具备的 60 多个。文件系统和外部 API 的工具 schema 从头到尾没有进入 LLM 的上下文。

### 12.5 另一种模式:基于上下文自动过滤

由 Host 应用本身监听对话上下文,自动判断需要哪些工具,然后驱动 Server 注册/注销。LLM 不需要显式调用元工具——工具列表对 LLM 是**无感地**变化的。

复杂度集中在 Host 端的路由逻辑(谁来判断、按什么规则、何时触发),Server 和 Client 之间的协议交互跟元工具模式完全一致。
...
| 维度        | 说明                                                        |
| --------- | --------------------------------------------------------- |
| **适用规模**  | 单个 Server 暴露的工具超过约 20 个,且天然可按领域分组(数据库、文件、外部 API 等)        |
| **适用任务**  | 任意一次任务通常只用得到其中一两类工具                                       |
| **首次延迟**  | 首次使用某类工具时多一轮往返(元工具调用 + 列表更新)                              |
| **认知负担**  | 元工具本身也占用 LLM 注意力——它要先意识到"应该去加载什么"                         |
| **客户端要求** | Client 必须正确处理 `list_changed` 通知并重新拉取列表,但并非所有 MCP 客户端实现都支持 |
