...
如果写出了"在遍历函数里,对每个节点再调用另一个递归函数"的代码(例如朴素解二叉树直径:遍历每个节点,对每个节点算一次最大深度),复杂度会从 O(n) 恶化到 O(n²)。
...
3. **直接写函数签名**:要不要返回值?返回什么?参数带什么?返回值装不下答案就加外部变量。签名写出,模式/通道/时机全部尘埃落定,剩下的是填空。
...
> [!warning] 易错点
>
> - 110 的朴素写法(每个节点调一次求高度函数)是 O(n²)"递归套递归",上面的 -1 编码法才是 O(n)。
