...
每个题型按统一结构展开：先用 **「看到这种题→就是它」**（识别信号，母题与高频度并在其中）认出题型，再用一段正文讲透**本质**（原理与通用解法合并），随后给模板与代表题；每道题特有的坑，就近放在它的代码块下面。
...
> [!tip] 看到这种题 → 就是它
> **数组 / 字符串有序**（或可排序），要找“一对位置”满足某条件：两数之和、三数之和、首尾配对求最优（最大面积、最远距离）、回文判断、原地反转。题面常含“有序数组”“两个数”“一对”“回文”“容器/面积”。母题 L11 盛水最多的容器（中），面试高频 ★★★。

两个指针一头一尾，每比较当前这一对，就能认定其中一端“再配谁都不会更优”，把它排除、指针内移——一次比较排除一整列配对，区间单调收缩，O(n) 扫完。这条循环不变量（最优解一定还在 `[left, right]` 里尚未排除的位置对中）对所有对撞题都成立；变的只有**比较什么量、据此移哪一端**，这才是每道题要现定的内容。
...
> [!example] 例 2 · L11 盛水最多的容器（中）｜招牌题｜HOT 100
...
> [!example] 例 3 · L15 三数之和（中）｜进阶·面试最高频｜HOT 100
...
> [!tip] 看到这种题 → 就是它
> 操作对象是**链表**，要定位某个“特殊位置”：中点、环、环入口、倒数第 k 个；或单趟判断有没有环。题面常含“链表”“中间节点”“环”“倒数第 k”“相交”。母题 L142 环形链表 II（中·找环入口），面试高频 ★★★。

不靠排序、不靠边界区，靠**速度差导出的数学关系**：两指针都从链头出发，慢的每次一步、快的每次两步，快到链尾时慢指针恰在中点；判环则让两者在环里相遇，相遇后把一个指针拨回链头、两者同速前进会在环入口再次相遇（推导见下方 callout）；倒数第 k 则让快指针先走 k 步、再两者同步。这套“靠速度差 / 定距定位”对所有快慢链表题都成立；变的只有**怎么跑、从哪起步、相遇后做什么**。
...
> 一切快慢链表题的母题：慢走一步、快走两步，快到尾时慢恰在中点。偶数节点默认返回靠后的中点。
...
> [!example] 例 2 · L141 / L142 环形链表（易 / 中）｜招牌题·Floyd 判圈｜HOT 100
...
> [!example] 例 3 · L19 删除倒数第 N 个（中）｜定距变体｜HOT 100
...
> [!tip] 看到这种题 → 就是它
> 要**原地改造数组**：删除某值、去重、把某类元素搬到一端；要求 O(1) 额外空间、不开新数组。题面常含“原地”“移除”“去重”“移动”“返回新长度”。母题 L27 移除元素（易），面试 ★★（常作更难题的子步骤）。

循环不变量是：slow 划出的那段 `[0, slow)` 是“已处理好的成品区”，区外还没碰；fast 向前逐个探，每步只决定当前元素要不要并入成品区——要则写入、边界进一格，不要则边界不动；fast 扫完，成品区即答案。边界只进不退 → O(n)、O(1)。公理就这一句、别当铁律：一道题只需定下面**两个变量**，其余（成品区开闭、指针初值、写入顺序、返回值）全被推导出来。
...
> [!example] 例 3 · L283 移动零（易）｜HOT 100
...
> [!example] 例 4 · L206 反转链表（易）｜链表版同向读写｜HOT 100
...
> [!tip] 看到这种题 → 就是它
> 有**两个序列**（两个有序数组、两条链表），要合并、求交集、判断包含关系。每个序列派一个指针，都只往前走。母题 L88 合并两个有序数组（易），面试 ★★（也是归并排序的核心步骤）。

两个指针各扫一个序列、都只同向前进，比较当前两个元素决定谁前进、谁并入结果，直到任一指针走完。它和对撞同样依赖“有序”，但两指针落在**两个不同序列**上；不回退的依据是“两序列都有序，较小者一旦并入结果就再没有更小的来抢它的位置”。这条对所有双序列归并题都成立；变的只有**比较后做什么、往哪填、要不要收尾**。
