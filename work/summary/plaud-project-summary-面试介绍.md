...
> **补充细节**（如果继续追问）：
>
> - **State 共享机制**：子 Agent 创建时传入与主 Agent 相同的 `state_schema=SummaryState`，保证两边读写同一套字段（`structure_planner`、`final_summary` 等）
> - **并发写合并**：多个并行任务写同一字段（如 `extracted_sections`）时，用 `Annotated[dict, merge_dicts]` 声明的 reducer 合并，而不是覆盖
> - **独立 middleware 栈**：子 Agent 有自己的 middleware（如 `AgentLoggingMiddleware(agent_name="subagent")`），独立于主 Agent
