...
### 与 Celery / Airflow 的区别：可靠性保障的单位不同

任何多步业务流程都由两部分组成：**每一步各自的执行**，和**步骤之间的衔接逻辑**（上一步结果是什么、现在走到哪、下一步做什么）。三个框架的差别在于把可靠性承诺立到哪一层：

- **Celery（任务队列）**：只保「每一步」。单个任务会被投递到 Worker 执行、失败可重试，但衔接逻辑留在业务进程的内存里——进程在两步之间崩溃，没有任何组件知道还有条流程停在半路。想可靠，业务只能自己建状态表、写定时扫描来推进，等于手写一个简陋的 Temporal。
- **Airflow（批处理 DAG 调度器）**：比 Celery 多保一层粗粒度的流程状态——DAG run 记录每个节点的成败，失败可从断点重跑。但保住的只有「哪些节点成功了」；流程必须预先画成静态 DAG，且面向定时批处理，「等一个外部事件再继续」「运行时决定循环几次」「中途暂停数月」都不在它的模型里。
- **Temporal**：保住程序的完整运行时状态——执行到哪一行、局部变量的值、收到过哪些 Signal，全部通过事件历史可恢复。衔接逻辑本身就是 Workflow 代码，所以承诺能立到整条流程级：**代码一定执行到 return**，流程可以是任意形状（顺序、并发、循环、条件、长时间等待）。

以「扣款 → 等待发货（最长 7 天）→ 发确认邮件」为例：Celery 中三个任务各自可靠，但「扣款成功后要等发货、发货后要发邮件」这条主线没人保管；Airflow 表达不了「等最长 7 天的外部事件」；Temporal 中这就是一个函数，`await` 等 7 天不占任何 Worker 资源，期间崩溃、部署、换机器都不影响，恢复后从原位置继续。


| 维度         | Temporal                  | Celery / Airflow（任务调度框架）          |
| ---------- | ------------------------- | -------------------------------- |
| 保障单位       | 整条流程（执行位置、局部变量、Signal 全可恢复） | 单个任务（会被执行、失败可重试）                 |
| 衔接逻辑放在哪    | 就是 Workflow 代码，随事件历史持久化    | Celery：业务进程内存；Airflow：静态 DAG     |
| 失败恢复       | 重放事件历史，从代码中断处继续           | Celery：重试该任务；Airflow：从失败节点重跑 DAG |
| 长任务 / 事件等待 | 数月等待、等外部 Signal 天然支持      | 不在模型内                            |
| 编排         | 直接写代码（顺序、并发、循环、条件）        | DAG 或任务链                         |
