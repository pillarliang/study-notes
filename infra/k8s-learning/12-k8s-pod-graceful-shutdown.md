...
> 前置知识：了解 [[02-k8s-core-concepts|Pod 和 Deployment]] 的基本概念。
>
> **核心问题**：在 Kubernetes 中运行长耗时任务（如 AI 摘要生成、音视频转码）时，服务重启/滚动更新会导致正在执行的任务被强制终止。如何做到——正在执行的存量任务不被中断、新发起的任务不丢失或失败、整个过程对客户端完全透明？
...
以下从第一性原理出发，逐层拆解优雅终止的每一层机制。

---

## 第一层：滚动更新策略 — 新 Pod Ready 前不杀旧 Pod

> [!example] 先看真实配置：generic-deployer 的默认滚动更新策略
> 公司 80+ 微服务共用同一套 Deployment 模板：
> ```yaml
> # 来源：deploy/generic-deployer/templates/deployment.yaml
> strategy:
>     type: RollingUpdate
>     rollingUpdate:
...
> 对应的默认值：
> ```yaml
> # 来源：deploy/generic-deployer/values.yaml
> updateStrategy:
>   maxUnavailable: 0
>   maxSurge: "25%"
> ```
> `maxUnavailable: 0` 意味着滚动更新时不允许任何 Pod 不可用，新 Pod Ready 后才会终止旧 Pod。
...
以 `replicas: 2` 为例，`maxSurge: 25%` 向上取整 = 1，更新流程为：
...
**核心矛盾：** 路径 A 和路径 B 是同时启动的。路径 A 中 API Server 虽然立即修改了 Endpoints，但 kube-proxy 和 Ingress Controller 需要 1~3 秒才能感知变更并更新路由规则，此期间新请求仍然会被路由到旧 Pod。而路径 B 中如果没有 preStop hook，SIGTERM 会瞬间到达应用进程，应用立即开始拒绝请求——但此时流量摘除还没传播完，仍有新请求涌入，这些请求就会失败。
...
> [!example] 先看真实配置：generic-deployer 的默认 preStop hook
> 公司所有服务的部署模板默认内置了 preStop hook：
> ```yaml
> # 来源：deploy/generic-deployer/values.yaml
> lifecycle:
>   preStop:
...
> {{- end }}
> ```
> 像 `plaud-project-summary` 这样的关键服务，还会在每个环境的 values 文件中显式重复声明这段配置，作为防御性做法——万一有人改了 Chart 默认值也不受影响。

preStop hook 是 K8s 提供的 Pod 生命周期钩子，**在 SIGTERM 发送之前执行**。其作用是在路径 B 中插入一个延迟，让应用在传播窗口内保持完全健康，等路径 A 先完成再开始真正的终止流程：
...
```ini
# 来源：plaud-project-summary 容器内 supervisord.conf
[program:api]
...
```python
# 来源：plaud-project-summary 应用代码（简化）
_shutting_down = False
...
> [!example] 先看真实配置：不同服务的 terminationGracePeriodSeconds 对比
...
>
> 同一个服务（如 `plaud-project-recommendation`）的不同组件可以有不同的优雅期——API 服务只需 30 秒，而后台 task-consumer 需要 1800 秒（30 分钟）。
...
> [!example] 先看真实配置：plaud-project-summary 的完整优雅关闭配置
> plaud-project-summary 是 AI 摘要生成服务，单个任务可能运行 10 分钟，对优雅关闭的要求最为苛刻：
>
> ```yaml
> # 来源：deploy/plaud-project-summary/values/ap-northeast-1/staging/main.yaml
> deployer:
...
> ```
>
> 这个 values 文件覆盖了检查清单中 K8s 层面的所有配置项。`replicaCount: 2` + 默认的 `maxUnavailable: 0` 确保滚动更新时始终有 Pod 在服务；`terminationGracePeriodSeconds: 600` + `preStop sleep 10` 组合满足了四层超时中的前两层。剩余两层（supervisord `stopwaitsecs` 和应用层代码）在 Docker 镜像内部配置。
